...
<body>
<div id="loader">
<div class="img"><img src="/herramientas/reporte/animat-printer-color.gif" height="96"></div>
	...
	fetch('request.php?title=<?=$title;?>&url=<?=$url;?>')
		...
		.then(data => {
			if (data && data["pdf"]) {
				...
				ifr.style.display="block";
				if(printPdf=='1'){
					ifr.src="/herramientas/reporte/web/viewer.php?file=../"+data.pdf+"#locale=es-MX";					
				}else{
					ifr.src="/herramientas/reporte/"+data.pdf;
				}
			}
			else {
				document.querySelector("#loader").style.display="none";
