        ...
        else
        {
          switch (mTipos[j])
          {
			...
			case 'N2': // 2 DECIMALES
              ...
              xvalor = mask.format(parseFloat(unformat(xvalor)).toFixed(2));
              oCell.innerHTML = xvalor;
              break;

            case 'N3':
			  var mask = new Mask('#,###.###', 'number');
              xvalor = mask.format(parseFloat(unformat(xvalor)).toFixed(3));
              oCell.innerHTML = xvalor;
              break;

            case 'N4':
			var mask = new Mask('#,###.####', 'number');
              xvalor = mask.format(parseFloat(unformat(xvalor)).toFixed(4));
              oCell.innerHTML = xvalor;
              break;

            case 'N5':
			var mask = new Mask('#,###.#####', 'number');
              xvalor = mask.format(parseFloat(unformat(xvalor)).toFixed(5));
              oCell.innerHTML = xvalor;
              break;

            case 'N6': // 3 DECIMALES
			var mask = new Mask('#,###.######', 'number');
              xvalor = mask.format(parseFloat(unformat(xvalor)).toFixed(6));
              oCell.innerHTML = xvalor;
              break;
      	...
      	for (var j in this.arrayPie[i])
      	{
            ...
            else
            {
	        	switch (xtipo)
	            {
	              ...
	              this.pies[xcampo].alt=xvalor;
	              this.pies[xcampo].title=xvalor;
        	}
    	}
	}
}
