...
class DB{
	...
	function config_sql()
	{
        ...
        $q = $this->sql;
        echo $q;
        $q = str_ireplace('select','SELECT',$q);
        $q = str_ireplace('from','FROM',$q);

        preg_match("/SELECT (.*) FROM (.*)/", $q, $matches);

        $limsep=split('LIMIT',$matches[2]);

        $q = 'SELECT COUNT(*) CANTIDAD FROM '.$limsep[0];
	...
	function cuenta_registros()
	{
	    if(strlen($this->sql_count))
	    {
            ...
            $this->next_record();
            $this->registros = $this->Record['CANTIDAD'];
