...
store.readFromFile('./sessions/' + puerto + '_store_multi.json')

// save every 10s
setInterval(() => {
	store.writeToFile('./sessions/' + puerto + '_store_multi.json')
}, 10_000)

const { state, saveState } = useSingleFileAuthState('./sessions/' + puerto + '_auth_info_multi.json')
	...
	*/
	...
	sock.ev.on('connection.update', (update) => {
		...
		if (update.qr) {
			...
			qr_svg.pipe(file.createWriteStream('./sessions/'+puerto+'.svg'));				
		}
		//let qr_svg = QR.image(update.qr, { type: 'svg', margin: 4 });
		/*
...
app.get('/qr', (req, res) => {
	var qrimg = "./sessions/"+puerto+".svg";
	file.exists(qrimg, (exists) => {
		if (exists) {
			res.set({'Content-Type': 'image/svg+xml'});
			res.sendFile(path.join(__dirname, '/.'+qrimg));
		}
		else {
			setTimeout(()=>{
				res.redirect("/qr");
