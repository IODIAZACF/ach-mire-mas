...
@mkdir(__DIR__ . '/mensajes/www/pendientes', 0777, true);
@mkdir(__DIR__ . '/mensajes/www/error', 0777, true);
@mkdir(__DIR__ . '/mensajes/www/enviados', 0777, true);
...
while(1){	
	...
	$sql = "SELECT * FROM D_CHAT WHERE ESTATUS='PENDIENTE' AND UPPER(ORIGEN)='" . strtoupper('sistemas24.com') . "' AND CANAL='WWW'  AND FECHA_ENVIO <= CAST('NOW' AS TIMESTAMP)";
	...
	for($i=0;$i<sizeof($xRecord);$i++)
	{
		...
		if($Record['XMENSAJE']==''){
			...
			file_put_contents(__DIR__ . '/mensajes/www/pendientes/' . $Record['ID_D_CHAT'] , $mensaje);	
			unset($resp);
			$estatus = null;
			exec(__DIR__ . '/www_send.php '. $Record['ID_D_CHAT'], $resp, $estatus);
