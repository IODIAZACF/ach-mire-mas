...
function buscar_grid(xgrid, xbusca){
	...
	if( !server_path ) {
		console.error('falta server_path ');
		return;
	}

	if( !db ) {
		console.error('falta db');
		return;
	}
...
function cargar_pagina(ruta, params, callback){
	...
	if(callback) {
		setTimeout( function(){ callback('OK'); }, 50 );
		//app.preloader.hide();
		 ...
		 if(xenviar=='1' && xValido)
         {
            ...
            if(xRequerido=='S' && $(obj).val()=='')
            {
                ...
                app.dialog.alert(xAlerta, '',function(){
					...
					if(callback) {
						setTimeout( function(){ callback( jQuery.parseJSON('{"tabla":{"estatus":"ERROR", "msg":"REQUERIDO", "campo":"' + campo.id + '"}}') )  ; }, 100);
...
function cargar_formulario(ruta, xbusca, callback){
	...
	app.request.get(ruta + '?'+rnd, function (data) {
		$$('#form-popup').html(data);
		
		var obj = $$('#form-popup').find('form');
		
		if(!xbusca || xbusca == '' || xbusca == '-1' ){
			if(xbusca == '-1') $$(obj).data('xbusca', xbusca);	
			
			console.log( $$(obj).data('xbusca') );
		} else {
			...
			//app.preloader.show();
			var url = server_path + "herramientas/genera_json/genera_json.php";
			var par = 'db=' + db + '&tabla=' + tabla + '&campos=*&busca=' + busca + '&xbusca=' + xbusca + '&operador==';
...
function cargar_opciones(JQObj, dValue, debug){
	
	var url = server_path + "herramientas/genera_json/genera_json.php";
	var xrnd  = Math.floor((Math.random() * 100000000) + 1);
	var params = 'db=' + db +'&tabla='+ JQObj.data('tabla') +'&campos=*';
		params += '&orden='+ JQObj.data('campo_orden');
	
	if(JQObj.data('padre')){
		params += '&busca='+ $('#' + JQObj.data('padre')).data('campo_guardar');
		params += '&xbusca='+ $('#' + JQObj.data('padre')).val();		
	}
	...
	JQObj.empty();
	//prompt(JQObj.attr('id'),  url +'?'+ params);
	enviar2(url, params, 'post', function(data){
		...
		if(data.tabla.registro){
			...
			$.each(data.tabla.registro, function(xtabla, xregistro) {
				...
				JQObj.append('<option value="'+ xregistro[JQObj.data('campo_guardar')] +'"'+ xselect +'>' + xregistro[JQObj.data('campo_mostrar')] +'</option>');
			});			
		}
		if(dValue && JQObj.data('hijo')){			
			cargar_opciones($('#' + JQObj.data('hijo')), localStorage.getItem(JQObj.data('hijo')), debug);
		}		
	});
