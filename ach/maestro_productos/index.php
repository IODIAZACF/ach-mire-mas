...
function ayuda_guardar(){
	...
	var contenido_delta = JSON.stringify(quill.getContents()); 
	

	var url = server_path + "maestro_productos/ayuda.php";
	var params = "db=demo24&tipo=" + tipo + "&origen=" + origen + "&objeto=" + objeto + "&contenido=" + encodeURIComponent(contenido) + "&contenido_delta=" + contenido_delta ;
...
function ayuda_editar( tipo, titulo, origen, objeto  ){
	...
	$.getScript( server_path + "herramientas/editor_quill/javascript/quill.min.js", function( data, textStatus, jqxhr ) {
		console.log( data ); // Data returned
		console.log( textStatus ); // Success
		console.log( jqxhr.status ); // 200
		console.log( "Quill Load was performed." );

		if(jqxhr.status != 200){
			console.error('Error al cargar el Editor Quill...!');
			return;
		...
		enviar(url, params, 'POST', function(data){
			...
			if(data.tabla.registro){
				...
				quill.setContents(JSON.parse(data.tabla.registro.CONTENIDO_DELTA));
			}
			...
			myTooltip = new jBox('Tooltip', {
				...
				onOpen: function(){
					...
					if(this.source[0].id){
						...
						this.setTitle( 'Ayuda ' + this.source.html().replace('<br>','') + '&nbsp; <button onclick="' + xfunc + '">editar</button>' );
